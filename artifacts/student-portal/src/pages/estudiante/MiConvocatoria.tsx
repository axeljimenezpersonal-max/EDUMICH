...
function ExamenCard({ examen, compacto = false, historial = false }: { examen: ExamenInscrito; compacto?: boolean; historial?: boolean }) {
  ...
  const showCountdown = !historial && diasRestantes <= 30 && diasRestantes > 0;
  const fecha = parseFecha(examen.fechaExamen);

  return (
    <div className={compacto
      ? 'p-4 flex items-center gap-4'
      : 'bg-white border border-stone-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4'}>
      {/* Fecha + hora del examen (el foco de la tarjeta) */}
      <div className={`${historial ? 'bg-stone-400' : 'bg-[var(--color-guinda-700)]'} text-white rounded-lg px-3 py-2 text-center min-w-[64px] flex-shrink-0`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-stone-500 mt-1">
          {compacto ? '' : `Etapa ${examen.etapa.clave} · `}{examen.dia === 'sabado' ? 'Sábado' : 'Domingo'} {formatFechaCorta(examen.fechaExamen)}
        </p>
        {!compacto && (
      ...
      {/* Estado + acceso al pase (sólo si ya está pagado y aún es vigente) */}
      <div className="flex flex-col items-end gap-1.5 shrink-0 text-right">
        <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${badge.cls}`}>
          {badge.label}
        </span>
        {historial && examen.calificacion != null && (
          <span className="text-[11px] font-bold text-stone-600">Calificación: {examen.calificacion}</span>
        )}
        {showCountdown && <span className="text-[11px] text-stone-400">Faltan {diasRestantes} días</span>}
        {!historial && examen.pagado && (
...
// ── Grupo por convocatoria (encabezado con sede + módulos adentro) ─────────

function GrupoConvocatoria({ clave, examenes, historial = false }: { clave: string; examenes: ExamenInscrito[]; historial?: boolean }) {
  const sede = examenes[0]?.sede?.nombre ?? null;
  return (
    <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-stone-100"
        style={{ background: historial ? '#fafaf9' : 'var(--color-crema-100)' }}>
        <div>
          <div className="text-[10px] font-bold uppercase tracking-widest" style={{ color: historial ? '#a8a29e' : '#b39a56' }}>
            Convocatoria {historial ? '· concluida' : 'inscrita'}
          </div>
          <div className="font-serif text-lg font-bold leading-tight" style={{ color: historial ? '#78716c' : 'var(--color-guinda-800)' }}>
            {clave}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-semibold">
          {sede && (
            <span className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1 text-stone-600 border border-stone-200">
              <MapPin className="w-3 h-3" /> {sede}
            </span>
          )}
          <span className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1 text-stone-600 border border-stone-200">
            {examenes.length} módulo{examenes.length === 1 ? '' : 's'}
          </span>
        </div>
      </div>
      <div className="divide-y divide-stone-100">
        {examenes.map((e) => <ExamenCard key={e.id} examen={e} compacto historial={historial} />)}
      </div>
    </div>
  );
}
    ...
    <EstudianteLayout>
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        {tieneExamenes && (() => {
          ...
          const porClave = new Map<string, ExamenInscrito[]>();
          for (const e of misExamenes) {
            const arr = porClave.get(e.etapa.clave) ?? [];
            arr.push(e);
            porClave.set(e.etapa.clave, arr);
          }
          const grupos = [...porClave.entries()].map(([clave, exs]) => ({
            clave,
            exs: [...exs].sort((a, b) => parseFecha(a.fechaExamen).getTime() - parseFecha(b.fechaExamen).getTime()),
            maxFecha: Math.max(...exs.map((x) => parseFecha(x.fechaExamen).getTime())),
          }));
          const proximos = grupos.filter((g) => g.maxFecha >= hoyUTC).sort((a, b) => a.maxFecha - b.maxFecha);
            ...
            <div className="space-y-5">
              {proximos.length > 0 && (
                <div>
                  ...
                  <div className="space-y-4">
                    {proximos.map((g) => <GrupoConvocatoria key={g.clave} clave={g.clave} examenes={g.exs} />)}
                  </div>
                </div>
              )}
              ...
              {anteriores.length > 0 && (
                <details className="group">
                  <summary className="flex cursor-pointer list-none items-center gap-2 rounded-xl border border-stone-200 bg-white px-4 py-3 font-semibold text-stone-700 hover:bg-stone-50 [&::-webkit-details-marker]:hidden">
                    <Clock className="w-4 h-4 text-stone-400" />
                    Exámenes anteriores
                    <span className="rounded-full bg-stone-100 px-2 py-0.5 text-[11px] font-bold text-stone-500">{nAnteriores}</span>
                    <span className="ml-auto text-xs text-stone-400 transition-transform group-open:rotate-180">▾</span>
                  </summary>
                  <div className="mt-3 space-y-4">
                    {anteriores.map((g) => <GrupoConvocatoria key={g.clave} clave={g.clave} examenes={g.exs} historial />)}
                  </div>
                </details>
              )}
            </div>
